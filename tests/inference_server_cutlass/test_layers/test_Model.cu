...
int main()
{
    ...
    const int target_len = 1;   // single decode step, just testing the Decoder
    ...
    // -------------------------
    // 6. Single Decoder step (no greedy decode loop) — just checking that
    //    the Decoder forward pass runs and produces sane logits.
    // -------------------------
    ...
    decoder->forward(
        ...
        next_token_logits,
        batch_size, seq_len, /*decoder_step=*/0,
        scale_x,
