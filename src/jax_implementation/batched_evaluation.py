...
def batched_evaluation(args):
    ...
    for batch in dataloader:
        ...
        best_guesses_gpu = predict_fn(params, inputs)
    ...
    n_correct = sum(p == g for p, g in zip(expressions, references))
    total = len(references)
    accuracy = 100.0 * n_correct / total

    print(f'Samples: {total}')
    print(f'Exact match: {n_correct}')
    print(f'Accuracy: {accuracy:.2f}%')

    n_shown = 0
    for i in range(total):
        if expressions[i] != references[i] and n_shown < 10:
            print(f'MISMATCH  input={factors[i]}  '
                  f'pred={expressions[i]}  gold={references[i]}')
            n_shown += 1


def main():
    args = get_arguments()
    batched_evaluation(args)
